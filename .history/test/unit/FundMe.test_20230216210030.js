...
describe("FundMe", async () => {
    ...
    describe("withdraw", async () => {
        ...
        it("allows us to withdraw money when multiple funder have donated.", async () => {
            ...
            // 3. array empty hunu paryo.. revert garnu paryo. s_funders array ko 0 th element retreive garna khojda.
            await expect(fundMe.s_funders()).to.be.reverted
            //4. harek account ko fund check garda- amount 0 aaunu paryo.
            for (i = 1; i < 6; i++) {
                let balance = await fundMe.s_addressToAmountFunded(
                    accounts[i].address
                )
